                      ...
                      <div class="panel-body">
                        <div class="row">
                         ...
                         <?php if(isset($_POST['hid'])){
							 ...
							 $id_req =  mysql_query("SELECT `users_password` FROM `users_master` WHERE `users_id` = '".$_SESSION['id']."'");
							 $id_data = mysql_fetch_array($id_req);
							 if($id_data['users_password'] != $pass){
								 echo "<span class='btn btn-danger'>Old Password Missmach</span>";
							 }else{
								 if($pas1==$pas2){
		mysql_query("UPDATE `eiem_project`.`users_master` SET `users_password` = '".$_POST['users_password']."' WHERE `users_master`.`users_id` = '".$_SESSION['id']."'");
